...
export async function prune(options = {}) {
  ...
  try {
    ...
    for (const entry of await readdir(root, { withFileTypes: true })) {
      const match = entry.isFile() && entry.name.match(/^([a-f0-9]{64})(?:\.json|\.(?:event|prompt|notice|repeat)\..+\.json)$/)
      if (!match) continue
      const file = join(root, entry.name)
      const session = sessions.get(match[1]) || { files: [], updatedAt: 0 }
      session.files.push(file)
      session.updatedAt = Math.max(session.updatedAt, (await stat(file)).mtimeMs)
